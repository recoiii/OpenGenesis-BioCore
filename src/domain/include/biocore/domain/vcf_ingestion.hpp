...
enum class VcfFieldType : std::uint8_t { integer, floating_point, flag, character, string };
enum class VcfNumberKind : std::uint8_t { fixed, alternate, reference_and_alternate, genotype, variable };
...
[[nodiscard]] VcfIngestionResult ingest_vcf(std::istream& input, const ReferenceGenome& reference);
